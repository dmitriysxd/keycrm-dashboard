...
async function fetchPages(endpoint, params, apiKey, maxPages) {
  ...
  while (page <= maxPages) {
    ...
    Object.assign(params, { page: page, limit: 50 });
    ...
    if (!res.ok) {
      ...
      throw new Error("HTTP " + res.status + " [" + endpoint + "]: " + body.substring(0, 200));
    }
    var json = await res.json();
    var items = json.data || [];
    all = all.concat(items);
    if (page >= (json.meta && json.meta.last_page ? json.meta.last_page : 1) || !items.length) break;
    page++;
  }
  return all;
}

// Вычисляем выручку заказа из его позиций (т.к. total_price может быть 0)
function orderRevenue(order) {
  var fromItems = (order.products || []).reduce(function(s, item) {
    return s + parseFloat(item.price || 0) * parseInt(item.quantity || 1);
  }, 0);
  // Если из позиций 0 — пробуем системные поля

-- v14 of sku_metrics: повертає buyers_count, який випадково загубився в 026.
--
-- Базується на 016 (повний набір колонок з buyers_count + 60-денний поріг
-- розриву циклу), плюс правки логіки статусу з 026:
--   1) 'new' дається 30 днів від keycrm_created_at НЕЗАЛЕЖНО від продажів і складу
--   2) 'archive' перевіряється РАНІШЕ за hit/good/slow/weak (current_stock=0 → одразу архів)
...
SELECT
  ...
  COALESCE(
    ...
    CASE
      ...
      --    продажів і складу. На 31-й день — звичайна логіка.
      WHEN keycrm_created_at IS NOT NULL
           AND keycrm_created_at >= NOW() - INTERVAL '30 days' THEN 'new'

      -- 2) Склад нульовий → 'archive'. Перевіряємо РАНІШЕ за weak/slow/etc.
      WHEN current_stock <= 0 THEN 'archive'

      -- 3) Звичайна сегментація за швидкістю продажів.
      ...
      -- 4) Склад є, продажів за 30 днів нема — 'dead'.
      ELSE 'dead'
    END
  ) AS status,
  CASE

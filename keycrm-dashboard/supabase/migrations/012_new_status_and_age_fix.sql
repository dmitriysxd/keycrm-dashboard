-- v9 of sku_metrics: refine "Новий" rule and age_days priority.
--
-- Problems with v8 (migration 011):
--
-- 1. Old SKUs with prior sales were getting "Новий" when stock bumped 0→1
--    (typically a customer return). detect_restocks() can't tell "fresh
--    batch arrived" from "single unit returned" — both look like 0→positive
--    transitions. The 30-day grace then mislabels SKUs like 01801
--    (sold_total = 83) as "Новий".
--
--    Fix: re-introduce sold_total = 0 requirement for "Новий". A SKU with
--    any sales history is, by definition, not new. After a real new batch
--    of an old SKU, normal V30 cascade kicks in immediately.
--
-- 2. age_days priority was wrong. v8 used last_restock_at first, which
--    means restocked old SKUs showed "age = 1 day". User wants
--    "age in catalog" = how long the SKU has existed.
--
--    Fix: use keycrm_created_at first (KeyCRM creation date), falling
--    back to first_stock_at, then last_restock_at. Sold-out SKUs still
--    get NULL → frontend renders "—".
--

-- v12 of sku_metrics: фікси логіки статусу.
--
-- ПРОБЛЕМИ КОРИСТУВАЧА (2026-05-20):
--
-- 1. SKU 03254 куплений 02.04 (24 шт), повністю проданий до 26.04. Сьогодні
--    current_stock=0. Має бути 'archive', але показується 'weak' — бо в
--    останні 30 днів ще є продажі (потрапляє в гілку sold_30d BETWEEN 1 AND 5
--    ДО гілки current_stock <= 0).
--    ФІКС: archive перевіряємо ПЕРШИМ. Якщо склад нульовий — це архів,
--    незалежно від того, що в межах 30 днів ще були продажі.
--
-- 2. SKU 03391-18 показується 'new', хоча вчора була продажа. Це наслідок
--    умови sold_total = 0 (для попадання в 'new'). Користувач хоче, щоб
--    'new' давався 30 днів від keycrm_created_at НЕЗАЛЕЖНО від продажів —
--    щоб новинки не "вибували" зі статусу при перших продажах.
--    ФІКС: 'new' — це просто "товар створено в KeyCRM <= 30 днів тому
--    І current_stock > 0". На 31-й день автоматично перекладається у
--    hit/good/slow/weak/dead за нормальними правилами.
--
-- 3. Нові товари, додані вчора в KeyCRM з нульовим залишком, не з'являються
--    в дашборді — їх ховає фільтр "Приховати архів" (бо archive за поточною
--    логікою = current_stock=0 + sold_total=0).
--    ФІКС: дамо їм статус 'new' навіть без current_stock. Так вони будуть
--    видні в таблиці 30 днів, поки не буде оприбуткування на склад.
...
SELECT
  ...
  COALESCE(
    manual_status,
    CASE
      -- 1) "Новий" — товар створено в KeyCRM ≤ 30 днів тому. НЕЗАЛЕЖНО від
      --    наявних продажів і навіть незалежно від наявності складу. Новинки
      --    залишаються в статусі 'new' рівно 30 днів від дня створення в CRM,
      --    щоб ти міг їх легко знайти і відпрацювати. На 31-й день — далі
      --    падають за загальною логікою.
      WHEN keycrm_created_at IS NOT NULL
           AND keycrm_created_at >= NOW() - INTERVAL '30 days' THEN 'new'

      -- 2) Якщо склад нульовий — це архів. Перевіряємо ДО hit/good/slow/weak.
      --    Раніше товар з sold_30d=2, але current_stock=0, неправильно ставав
      --    'weak'. Тепер: продано, на складі нуль → архів.
      WHEN current_stock <= 0 THEN 'archive'

      -- 3) Звичайна сегментація за швидкістю продажів за 30 днів.
      ...
      -- 4) Залишок є, продажів за 30 днів нема — 'dead'.
      ELSE 'dead'
    END
  ) AS status,
  CASE

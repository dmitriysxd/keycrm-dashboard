-- Rebuild sku_metrics so sales are joined by product_id only
-- (KeyCRM order line items reference product_id; their offer_id values
--  do not align with /products ids that we now use as the SKU key.)

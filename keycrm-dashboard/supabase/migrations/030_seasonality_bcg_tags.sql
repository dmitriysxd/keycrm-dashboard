-- v17 sku_metrics + сезонність + BCG ролі + actionable теги
--
-- ПЛАН (за побажанням користувача):
-- 1. Сезонний календар (підхід А): таблиця seasonal_periods. Multiplier
--    застосовується до абсолютних порогів статусу (hit/good/slow/weak).
-- 2. Індивідуальний baseline (підхід Б): relative_performance vs lifetime.
-- 3. BCG-матриця (підхід В): bcg_role на основі revenue share × trend.
-- 4. Actionable теги: масив 7 сигналів для UI.
-- 5. Категорія-aware квінтиль: для аналітики (зберігаємо як category_quintile).
--    Статус продовжує називатись "хіт" глобально, але в картці видно
--    "топ-20% в своїй категорії".

------------------------------------------------------------
-- 1. Таблиця сезонних періодів (з замовчанням під опт-ювелірку)
------------------------------------------------------------
CREATE TABLE IF NOT EXISTS seasonal_periods (
  id           SERIAL PRIMARY KEY,
  name         TEXT          NOT NULL,
  start_mm_dd  TEXT          NOT NULL,  -- формат 'MM-DD'
  end_mm_dd    TEXT          NOT NULL,
  multiplier   NUMERIC(4,2)  NOT NULL DEFAULT 1.0,
  category_id  BIGINT,                  -- NULL = усі категорії
  comment      TEXT,
  is_active    BOOLEAN       NOT NULL DEFAULT TRUE,
  created_at   TIMESTAMPTZ   NOT NULL DEFAULT NOW(),
  updated_at   TIMESTAMPTZ   NOT NULL DEFAULT NOW(),
  CONSTRAINT seasonal_periods_mm_dd_format CHECK (
    start_mm_dd ~ '^[0-1][0-9]-[0-3][0-9]$' AND
    end_mm_dd   ~ '^[0-1][0-9]-[0-3][0-9]$'
  )
);

-- Сезонний дефолтний набір — опт-ювелірка в Україні. Юзер потім підправить.
INSERT INTO seasonal_periods (name, start_mm_dd, end_mm_dd, multiplier, comment)
VALUES
  ('Новий рік (опт)',  '10-01', '12-15', 1.7, 'Опт закупає для НР і корпоративів'),
  ('8 березня (опт)',  '01-15', '03-05', 1.5, 'Опт закупає до Дня Валентина і 8 березня'),
  ('Випускні (опт)',   '03-15', '04-20', 1.3, 'Опт закупає до сезону випускних/весіль'),
  ('Літо (опт)',       '05-15', '08-31', 0.6, 'Низький сезон опту')
ON CONFLICT DO NOTHING;

------------------------------------------------------------
-- 2. Функція current_season_multiplier()
--    Повертає мультиплікатор для сьогоднішньої дати. Якщо є період для
--    конкретної категорії — він має пріоритет над all-categories.
------------------------------------------------------------
CREATE OR REPLACE FUNCTION current_season_multiplier(_category_id BIGINT DEFAULT NULL)
RETURNS NUMERIC
LANGUAGE sql STABLE
AS $$
  SELECT COALESCE(
    (
      ...
      WHERE is_active
        ...
        AND (
          (start_mm_dd <= end_mm_dd AND
           to_char(CURRENT_DATE, 'MM-DD') BETWEEN start_mm_dd AND end_mm_dd)
          OR
          (start_mm_dd > end_mm_dd AND
           (to_char(CURRENT_DATE, 'MM-DD') >= start_mm_dd OR
            to_char(CURRENT_DATE, 'MM-DD') <= end_mm_dd))
        )
      ORDER BY category_id NULLS LAST  -- категорійні мають пріоритет
      LIMIT 1
    ),
    1.0
  );
...
-- Допоміжна функція щоб з API дізнатись поточний сезон (його ім'я і мн).
CREATE OR REPLACE FUNCTION current_season_info()
RETURNS TABLE(name TEXT, multiplier NUMERIC, start_mm_dd TEXT, end_mm_dd TEXT, comment TEXT)
LANGUAGE sql STABLE
AS $$
  SELECT sp.name, sp.multiplier, sp.start_mm_dd, sp.end_mm_dd, sp.comment
  FROM seasonal_periods sp
  WHERE sp.is_active
    AND sp.category_id IS NULL  -- глобальний сезон
    AND (
      (sp.start_mm_dd <= sp.end_mm_dd AND
       to_char(CURRENT_DATE, 'MM-DD') BETWEEN sp.start_mm_dd AND sp.end_mm_dd)
      OR
      (sp.start_mm_dd > sp.end_mm_dd AND
       (to_char(CURRENT_DATE, 'MM-DD') >= sp.start_mm_dd OR
        to_char(CURRENT_DATE, 'MM-DD') <= sp.end_mm_dd))
    )
  ORDER BY sp.multiplier DESC
  LIMIT 1;
$$;

GRANT EXECUTE ON FUNCTION current_season_multiplier(BIGINT) TO authenticated, anon, service_role;
GRANT EXECUTE ON FUNCTION current_season_info() TO authenticated, anon, service_role;

------------------------------------------------------------
-- 3. Перебудова sku_metrics v17
------------------------------------------------------------
...
with_age AS (
  SELECT *,
    -- cycle age (від останньої партії, для статусу і UI)
    CASE
      ...
      ELSE NULL
    END AS age_days,
    -- lifetime (з моменту створення в каталозі — для V30/V90 nullify)
    CASE
      WHEN keycrm_created_at IS NOT NULL THEN EXTRACT(DAY FROM (NOW() - keycrm_created_at))::int
      WHEN first_stock_at IS NOT NULL THEN EXTRACT(DAY FROM (NOW() - first_stock_at))::int
    ...
    -- Сезонний мультиплікатор для категорії товару (1.0 якщо не в сезоні).
    current_season_multiplier(category_id) AS season_mult,
    -- Velocity (raw, без nullify)
    ROUND(sold_7d / 7.0, 3) AS v7_raw,
    ROUND(sold_30d / 30.0, 3) AS v30_raw,
    -- Lifetime velocity (на день фактичної наявності)
    CASE WHEN sold_total > 0 THEN ROUND(sold_total / vlife_days::numeric, 3) ELSE NULL END AS lifetime_v_raw
  FROM base
),
ranked AS (
  -- Квінтиль всередині категорії (тільки активні з продажами).
  -- Топ-квінтиль (5) = найкращі 20% в категорії. Bottom (1) = найгірші 20%.
  -- NULL для архіву і нових без продажів.
  SELECT *,
    ...
    -- Global revenue rank — для Hero tag і BCG ролей.
    -- NTILE(20) → 1 = топ-5%, 4 = топ-20%, ...
    CASE WHEN revenue_30d > 0 THEN
      NTILE(20) OVER (ORDER BY revenue_30d DESC NULLS LAST)
    END AS revenue_rank,
    -- Relative performance: фактична v30 vs історичний baseline.
    -- > 1.0 = продаємо краще ніж зазвичай, < 1.0 = гірше.
...
SELECT
  ...
  -- ── Основний статус (з сезонною поправкою) ───────────────
  -- Пороги множимо на season_mult. У високий сезон планка вища (легше
  -- продавати → строгий критерій хіта), у низький — нижча.
  ...
  ARRAY_REMOVE(ARRAY[
    ...
    -- Hero — топ-20% по виручці 30d
    CASE WHEN revenue_rank IS NOT NULL AND revenue_rank <= 4 THEN 'hero' END,
    -- Comeback — мав MAX(ordered_at) > 30 днів тому, але є sold_7d > 0
    ...
    -- Above baseline — продає на 20%+ краще за свою історичну норму
    CASE WHEN relative_performance IS NOT NULL AND relative_performance >= 1.2 THEN 'above_baseline' END,
    -- Below baseline — на 20%+ гірше за історичну норму

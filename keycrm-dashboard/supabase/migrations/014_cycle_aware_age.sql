-- v11 of sku_metrics: cycle-aware age_days.
--
-- Problem: SKU 00032 had three batches (Mar 2024, Dec 2024, Jan 2026)
-- separated by sold-out periods. User wants age = days since the CURRENT
-- batch arrived (Jan 14 2026 → ~115 days), not since the SKU first
-- existed (March 2024 → 772 days).
--
-- The previous formula fell back to keycrm_created_at because
-- last_restock_at was NULL — that field is only set when our ingest
-- observes a 0→positive transition, and the Jan 2026 batch arrived
-- before our ingest started.
--
-- Fix: detect the "last cycle start" from sales history. We already
-- compute cycle bounds for Vlife (cycle_summary CTE). Add a CTE
-- last_cycle_starts that takes the MAX cycle_start per product, then
-- use that as a fallback in age_days when last_restock_at is NULL.
--
-- Final age_days priority:
--   1. last_restock_at (explicit 0→positive transition we observed)
--   2. last cycle start from sales gaps (>30-day gap = new cycle)
--   3. first_stock_at (our first positive observation)
--   4. keycrm_created_at (KeyCRM creation date)
-- Sold-out SKUs return NULL → frontend renders "—".
--
...
last_cycle_starts AS (
  ...
  -- dates with no gap > 30 days. For multi-batch SKUs separated by

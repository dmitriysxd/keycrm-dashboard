-- Hotfix міграції 030: seasonal_periods недоступний при створенні matview.
--
-- ПРИЧИНИ (можуть бути обидві):
-- 1. Supabase цього проекту автоматично вмикає RLS на нових таблицях.
--    seasonal_periods створилась з RLS без policy → читання заборонено
--    навіть для service_role у деяких контекстах. Виправляємо
--    ALTER TABLE ... DISABLE ROW LEVEL SECURITY.
-- 2. Функція current_season_multiplier() не мала SECURITY DEFINER і
--    SET search_path, тому при виклику з matview-контексту не знаходила
--    public.seasonal_periods. Виправляємо обома.
--
-- Безпечно запускати кілька разів — все під IF NOT EXISTS / OR REPLACE.

------------------------------------------------------------
-- 1. Таблиця + RLS off + seed
------------------------------------------------------------
...
ALTER TABLE public.seasonal_periods DISABLE ROW LEVEL SECURITY;

-- Seed якщо порожньо
INSERT INTO public.seasonal_periods (name, start_mm_dd, end_mm_dd, multiplier, comment)
SELECT * FROM (VALUES
...
) AS v(name, start_mm_dd, end_mm_dd, multiplier, comment)
WHERE NOT EXISTS (SELECT 1 FROM public.seasonal_periods WHERE seasonal_periods.name = v.name);

------------------------------------------------------------
-- 2. Функції з SECURITY DEFINER + явним search_path + повністю
--    кваліфікованими іменами таблиць.
------------------------------------------------------------
DROP FUNCTION IF EXISTS public.current_season_multiplier(BIGINT);
CREATE OR REPLACE FUNCTION public.current_season_multiplier(_category_id BIGINT DEFAULT NULL)
RETURNS NUMERIC
LANGUAGE sql
STABLE
SECURITY DEFINER
...
DROP FUNCTION IF EXISTS public.current_season_info();
CREATE OR REPLACE FUNCTION public.current_season_info()
...
GRANT EXECUTE ON FUNCTION public.current_season_multiplier(BIGINT) TO authenticated, anon, service_role;
GRANT EXECUTE ON FUNCTION public.current_season_info() TO authenticated, anon, service_role;

------------------------------------------------------------
-- 3. Швидкий тест — функції мають повертати значення без помилок
------------------------------------------------------------
DO $$
DECLARE
  test_mult NUMERIC;
BEGIN
  SELECT public.current_season_multiplier() INTO test_mult;
  RAISE NOTICE 'current_season_multiplier() = %', test_mult;
END $$;

------------------------------------------------------------
-- 4. Перебудова sku_metrics (така ж як 030, але викликає
--    функцію через public. префікс — для впевненості)
------------------------------------------------------------
...
with_age AS (
  SELECT *,
    ...
    -- ВАЖЛИВО: public. префікс для функції щоб уникнути search_path проблем

-- v15 sku_metrics + reserved column + pg_cron diagnostic functions
--
-- ПРОБЛЕМИ КОРИСТУВАЧА:
--   1. V90 показує велосіті для товарів, які на складі менше 90 днів —
--      нечесно (товар не міг продаватись усі 90 днів, відображає занижену
--      швидкість). Те саме для V30 з товарами < 30 днів.
--   2. detect_restocks і chart показують "заказ" коли клієнт повертає
--      резервований товар (net quantity росте, але це не реальний приход).
--   3. "автообновление метрик срабатывает?" — потрібен спосіб перевірити.

------------------------------------------------------------
-- 1. Колонка reserved в stock_snapshots — щоб відрізняти
--    реальні приходи (total qty diff) від звільнення резерву.
------------------------------------------------------------
ALTER TABLE stock_snapshots ADD COLUMN IF NOT EXISTS reserved NUMERIC(10,2);
ALTER TABLE stock_snapshots ADD COLUMN IF NOT EXISTS total_quantity NUMERIC(10,2);
-- Для існуючих рядків ми не знаємо точно total / reserved окремо. Залишаємо
-- NULL — фронт/детектор робитимуть fallback на quantity (= net) як раніше.

------------------------------------------------------------
-- 2. SKU metrics v15 — velocity nullify для занадто молодих товарів.
------------------------------------------------------------
...
sales_dates AS (
  SELECT product_id, MIN(ordered_at)::date AS first_sold, MAX(ordered_at)::date AS last_sold
  FROM sales
...
creation_anchors AS (
  ...
  FROM skus s JOIN sales_dates sd ON sd.product_id = s.product_id
...
active_lagged AS (
  SELECT product_id, day, LAG(day) OVER (PARTITION BY product_id ORDER BY day) AS prev_day
  FROM all_active_days
),
cycle_marks AS (
  SELECT product_id, day,
    CASE WHEN prev_day IS NULL THEN 1 WHEN (day - prev_day) > 60 THEN 1 ELSE 0 END AS new_cycle
  FROM active_lagged
),
cycle_assigned AS (
  SELECT product_id, day, SUM(new_cycle) OVER (PARTITION BY product_id ORDER BY day) AS cycle_id
  FROM cycle_marks
),
cycle_summary AS (
...
with_age AS (
  -- Окремий шар, щоб вираз age_days був доступний для умов velocity nullify нижче.
  SELECT *,
    CASE
...
SELECT
  ...
  keycrm_created_at, last_restock_at, current_stock, stock_date, peak_stock_30d,
  ...
  -- V7: завжди показуємо (короткий період, age не критичний).
  ROUND(sold_7d / 7.0, 3) AS velocity_7d,
  -- V30: nullify якщо товар на складі < 30 днів — нечесно ділити на 30 коли
  -- товар був доступний 5 днів.
  CASE WHEN age_days IS NULL OR age_days >= 30
    THEN ROUND(sold_30d / 30.0, 3) ELSE NULL END AS velocity_30d,
  -- V90: nullify якщо товар на складі < 90 днів.
  CASE WHEN age_days IS NULL OR age_days >= 90
  ...
  COALESCE(manual_status, CASE
    ...
    ELSE 'dead'
  END) AS status,
  CASE WHEN sold_total = 0 THEN 'dead'
    ELSE CASE
...
------------------------------------------------------------
-- 3. Діагностична функція для pg_cron.
--    Дозволяє з API дізнатись чи активна задача refresh-sku-metrics
--    і коли вона востаннє виконувалась.
------------------------------------------------------------
CREATE OR REPLACE FUNCTION pg_cron_status()
RETURNS TABLE(
  jobname text, schedule text, active boolean,
  last_start_time timestamptz, last_end_time timestamptz, last_status text
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  FROM cron.job j
  LEFT JOIN LATERAL (
    SELECT start_time, end_time, status::text
...
EXCEPTION
  WHEN insufficient_privilege OR undefined_table THEN
    -- pg_cron extension не доступна — повертаємо порожній набір замість помилки.
    RETURN;
END;
$$;

-- Дозволяємо викликати з service_role (через PostgREST/Supabase JS client).
GRANT EXECUTE ON FUNCTION pg_cron_status() TO authenticated, anon, service_role;

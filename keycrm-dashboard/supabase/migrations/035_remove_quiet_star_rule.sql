-- Migration 035: прибрати "тиху зірку" з логіки статусу SKU.
--
-- ПРИЧИНА: правило було
--   WHEN sold_30d >= 10 * season_mult AND st30_calc >= 0.7 THEN 'hit'
-- На практиці воно піднімало в "Хіт" товари з 10–14 продажами/міс лише тому,
-- що склад був майже розпроданий. Юзер каже — це не валідне визначення
-- хіта: 12 шт/міс це "Повільний", а не "Хіт", незалежно від того, чи
-- розпродали все.
--
-- Тепер статуси визначаються чисто по швидкості продажів за 30 днів:
--   sold_30d >= 30 * season_mult → hit
--   sold_30d >= 15 * season_mult → good
--   sold_30d >= 6  * season_mult → slow
--   інакше                       → weak
--
-- Решта матвью (BCG, tags, archive_tier, age_days, тощо) без змін.
...
SELECT
  ...
  -- ЗМІНЕНО (035): прибрано правило "тихої зірки"
  --   WHEN sold_30d >= 10 * season_mult AND st30_calc >= 0.7 THEN 'hit'
  -- Тепер чисто по швидкості продажу за 30 днів.
...
-- Перший рефреш — щоб дані з'явилися одразу.

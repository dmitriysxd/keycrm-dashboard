-- v12 of sku_metrics: cycle gap threshold 30 → 60 days.
--
-- Bug after migration 014: SKU 00063 (continuous in stock since Mar
-- 2024) showed age=198 days instead of ~770. Sales for this SKU have
-- occasional 30-50 day gaps (low-volume periods with no buyers),
-- which the 30-day threshold mistook for "sold out and restocked".
--
-- Fix: raise the cycle gap threshold to 60 days. Real batch separations
-- in user's history have multi-month gaps (4+ months for 00032, 81 days
-- for 01748). Low-volume continuous SKUs have at most ~50 day dry
-- spells. 60 days cleanly separates the two patterns.
--
-- Affects both age_days (last_cycle_start) and Vlife (sales_active_days)
-- since both derive from the same cycle_summary CTE. For continuous
-- low-volume SKUs Vlife denominator now correctly counts the full span;
-- for true multi-batch SKUs cycles still get split.
--

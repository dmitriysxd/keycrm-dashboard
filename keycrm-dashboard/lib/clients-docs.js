...
const METRIC_DOCS = {
  ...
  velocity_trend: {
    ...
    formula: "ratio = recent_interval / prior_interval. ratio < 0.8 → accelerating; > 1.25 → decelerating; інакше stable",
    period: "Порівняння всередині історії клієнта (без календарних вікон)",
    freshness: "Щоночі",
    notes: "Якщо recent або prior null — velocity null (недостатньо замовлень для аналізу).",
  },
  ...
  overdue: {
    ...
    short: "Клієнт прострочив свій цикл закупки",
    formula: "recency_days > avg_interval_days × 1.5",
    period: "Моментальний замір",
    freshness: "Щоночі",
    notes: "Тільки для клієнтів з 2+ замовленнями (інакше цикл = null). Рядок підсвічується червоною смужкою зліва.",
  },
  ltv: {
    id: "ltv",
    label: "LTV (Lifetime Value)",
    short: "Сумарна виручка від клієнта",
    formula: "= monetary (за всю історію, з урахуванням знижок)",
    period: "За всю історію",
    freshness: "Щоночі",
    notes: "Зараз показуємо ІСТОРИЧНИЙ LTV. Прогнозний LTV (з урахуванням churn × очікувана кількість майбутніх замовлень × AOV) — TODO.",
  },
  churn_pct: {
    id: "churn_pct",
    label: "Churn (ризик відтоку)",
    short: "Ймовірність втрати клієнта в %",
    formula: "Зважена сума 6 факторів, cap 0–95%. Дивись детальну розкладку у вкладенні churn_factors.",
    period: "Комбінує різні вікна — див. таблицю факторів",
    freshness: "Щоночі",
    notes: "Колірний код: 0–20% зелений, 20–40% сірий, 40–70% жовтий, 70–95% червоний.",
  },
  churn_factors: {
    id: "churn_factors",
    label: "Фактори churn",
    ...
    factors: [
      ...
      { name: "Velocity сповільнюється",       weight: "+10%", period: "останні 3 інтервали vs ранні", trigger: "recent_interval / prior_interval > 1.3" },
      { name: "Свіжі негативні заметки",       weight: "до +15%", period: "останні 30 днів", trigger: "1+ заметка з outcome 'не відповідає' / 'відмова' / 'не цікаво зараз'. Вага = min(15, кількість × 8)" },
    ],
  },
  ...
  notes: {
    ...
    period: "Зберігаються назавжди; в churn враховуються тільки за останні 30 днів",
    notes: "Outcomes 'не відповідає' / 'відмова' / 'не цікаво зараз' додають вагу в churn-ризик.",
  },
};
